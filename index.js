...
var Selectables = (function () {
    function Selectables(elements) {
        _classCallCheck(this, Selectables);

        // accepts a list or query of items to deem selectable
        ...
        this.startX = this.startY = this.deltaX = this.deltaY = this.transX = this.transY = this.rafID = this.dragStartHandler = this.dragMoveHandler = this.dragEndHandler = null;

        this.isKeydown = this.isDragging = false;

        this.init();
    }

    _createClass(Selectables, {
        init: {
            value: function init() {
        ...
        findSelectables: {
            value: function findSelectables() {
        ...
        updateDragSelect: {
            value: function updateDragSelect() {
                var _this = this;
        ...
        addSelectable: {
            value: function addSelectable() {}
        },
        removeSelectable: {
            value: function removeSelectable() {}
        },
        selectOne: {
            value: function selectOne(el) {
                el.classList.add("selected");
                //el.dispatchEvent('selected');
            }
        },
        deselectOne: {
            value: function deselectOne(el) {
                el.classList.remove("selected");
                //el.dispatchEvent('deselected');
            }
        },
        toggleOne: {
            value: function toggleOne(el) {
                console.log("test");
        ...
        setSVGAtts: {
            value: function setSVGAtts(el, atts) {
                if (atts !== undefined) {
                    for (var k in atts) {
        ...
        createSVG: {
            value: function createSVG(tag, atts) {
                var el = document.createElementNS("http://www.w3.org/2000/svg", tag);
                return this.setSVGAtts(el, atts);
            }
        },
        getPropPrefix: {
            value: function getPropPrefix(prop) {
